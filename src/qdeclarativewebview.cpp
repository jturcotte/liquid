...
void QDeclarativeWebView::findNext(const QString& text)
{
    d->view->findText(text, QWebPage::FindWrapsAroundDocument);
}

void QDeclarativeWebView::findPrevious(const QString& text)
{
    d->view->findText(text, QWebPage::FindBackward | QWebPage::FindWrapsAroundDocument);
}
...
QString QDeclarativeWebView::toolTipText() const
{
    return d->toolTipText;
}

QPoint QDeclarativeWebView::toolTipPos() const
{
    return d->toolTipPos;
}
...
void GraphicsWebView::hoverMoveEvent(QGraphicsSceneHoverEvent* event)
{
    QGraphicsWebView::hoverMoveEvent(event);
    if (!parent->d->toolTipText.isEmpty()) {
        parent->d->toolTipText = QString();
        emit parent->toolTipTextChanged();
    }
    parent->d->toolTipPos = event->pos().toPoint();
    parent->d->toolTipTimer.start(1000, parent);
}

void QDeclarativeWebView::timerEvent(QTimerEvent* event)
{
    if (event->timerId() == d->toolTipTimer.timerId()) {
        d->toolTipTimer.stop();
        QWebHitTestResult hit = page()->mainFrame()->hitTestContent(d->toolTipPos);
        if (!hit.alternateText().isEmpty()) {
            d->toolTipText = hit.title();
            emit toolTipTextChanged();
            emit toolTipPosChanged();
        }
    } else
